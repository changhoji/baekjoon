...
ll makeSegTree(int node, int left, int right) {
    ...
    seg[node] *= makeSegTree(2*node, left, mid);
    seg[node] *= makeSegTree(2*node+1, mid+1, right);
    seg[node] %= DIV;

    return seg[node];
}

ll updateSegTree(int node, int idx, int left, int right, ll num) {
    if (idx < left || idx > right) return seg[node];
    if (left == right) return seg[node] = num;

    int mid = (left + right) / 2;

    return seg[node] = (updateSegTree(node*2, idx, left, mid, num)*updateSegTree(node*2+1, idx, mid+1, right, num))%DIV;
}
